...
function Assert-GrammarPayload(
    ...
    $longestName = ($missing | Measure-Object -Property Length -Maximum).Maximum
    $buffer = [byte[]]::new(1024 * 1024)
    $overlap = ''
    while ($missing.Count -gt 0 -and ($read = $Stream.Read($buffer, 0, $buffer.Length)) -gt 0) {
        $payloadText = $overlap + [System.Text.Encoding]::Latin1.GetString($buffer, 0, $read)
        foreach ($fileName in @($missing)) {
            if ($payloadText.Contains($fileName, [System.StringComparison]::Ordinal)) {
                [void]$missing.Remove($fileName)
            }
        }
        $overlapLength = [Math]::Min($longestName - 1, $payloadText.Length)
        $overlap = $payloadText.Substring($payloadText.Length - $overlapLength)
    }
    foreach ($fileName in $missing) {
        Assert-Artifact `
            -Condition $false `
...
foreach ($rid in $manifest.rids) {
    ...
    try {
        ...
        $infrastructureStream = $infrastructureEntries[0].Open()
        try { Assert-GrammarPayload $infrastructureStream $rid $packageName }
        finally { $infrastructureStream.Dispose() }
    }
    finally {
        $archive.Dispose()
    }
...
try {
    foreach ($rid in $manifest.rids) {
        ...
        try { Assert-GrammarPayload $binaryStream $rid $packageName }
        ...
        if ($hasReleaseArchives) {
            ...
            if ($rid.os -ceq 'win32') {
                ...
                try {
                    ...
                    $releaseStream = $releaseEntries[0].Open()
                    try {
                        $releaseHash = [Convert]::ToHexString(
                            [System.Security.Cryptography.SHA256]::HashData($releaseStream)).ToLowerInvariant()
                    }
                ...
                $releaseEntries = @(Read-UstarGzipArchive -archivePath $releasePath -hashEntryNames @([string]$rid.binary))
                ...
                $releaseHash = $releaseEntries[0].Sha256
            }
