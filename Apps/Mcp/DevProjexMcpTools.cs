	...
	McpAgentJournal? journal = null)
{
	...
	[Description(
		"Lists local projects, profiles, and baseline filters. Use it to choose a root or inspect policy; use get_tree for structure. Returns indexes, protected names and paths, root types, profiles, and Git/exclusion policy. A local project accepts a unique listed name, #index, or absolute path returned by list_projects; remote URLs require opt-in. No parameters.")]
	...
	[Description(
		"Returns filtered structure without contents. Use it to find files by name; use analyze instead for size and token metrics, or pack_context for multi-file content. Returns Markdown/text/JSON/XML within 2,000 lines and 50,000 characters. paths selects literal locations; include_patterns selects names or directories in one call, e.g. include_patterns=[\"src/middleware/{powered-by,body-limit,bearer-auth}/**/*handler*.ts\"]. format=markdown|text|json|xml; max_depth=0..1000 counts levels below the project root; git_scope and filters only narrow.")]
	...
	[Description(
		"Measures a selection before packaging: protected metrics, estimates, document size, and largest files. Use it to choose pack_context filters or max_tokens; use get_tree for structure and pack_context for content. Returns measured and estimated metrics without file bodies. Key parameters: detail=full|compact|signatures, top_files, git_scope, paths, include_patterns, exclude_patterns, profile, and max_file_bytes. Last detail_by_pattern match wins. max_tokens reports pack_context's greedy admission; rank and focus require it.")]
	public Task<CallToolResult> Analyze(
		...
		RunProjectAsync("analyze", request, async () =>
		{
			...
			{
				secrets = "always",
				privateData = McpProjectService.IsPrivateDataHidden(plan) ? "enabled" : "disabled"
			};
			...
			var formattedAdmissionReport = admissionBudget is null
				? null
				: McpSpotlight.Wrap(FormatTokenBudgetReport(admissionBudget));
	...
	[Description(
		"Builds multi-file project context. Use it after get_tree, search_project, or analyze; use get_file for one file. Returns inline untrusted project data, or pack_id plus a preview above 50,000 characters for read_pack. Key parameters: detail, view=tree|content|tree-content, format, rank, focus, max_tokens, git_scope, and detail_by_pattern. detail=full still transforms content. expand_related adds resolved selected neighbours. focus requires rank=importance; max_tokens admits greedily by estimate. Last pattern override wins.")]
				...
				if (pack.Characters <= MaximumInlinePackCharacters)
				{
					...
					if (inlineMessage.Length <= MaximumInlinePackCharacters)
					{
						...
						journal?.RecordDeliveredPaths(
							plan.SourceRoot,
							deliveredPackFiles);
						if (writeResult?.TokenBudget?.SkippedFileCount > 0)
							journal?.RecordNotice(AgentJournalNoticeCodes.BudgetSkipped);
	...
	[Description(
		"Reads a stored result page created by pack_context, search_project, or related_files. Use its pack_id; use pack_context instead when no valid stored result exists. Returns untrusted data up to 1,000 lines or 50,000 characters with trusted continuation or range notes. Protection changes invalidate stored content; selection-only changes retain it with a warning. Ranges and columns are inclusive and 1-based.")]
		...
		CancellationToken cancellationToken)
	{
		...
		async Task<CallToolResult> ReadAsync()
		{
			...
			if (journalContext is not null)
				journal?.RecordStoredContext(journalContext.SourceRoot, journalContext.Revision);
			if (liveContext?.RefreshStoredResult(packs.GetLiveContext(packId)) == true)
	...
	public Task<CallToolResult> SearchProject(
		...
		RunProjectAsync("search_project", request, async () =>
		{
			...
			var declarationPreview = McpSearchSymbols.SelectDeclarationBodyPreview(
				...
				regex);
			var declarationReadContext = CreateDeclarationReadContext(plan);
			var bodyLayout = PlanSearchDeclarationBody(
				rendered,
				symbols.Declarations,
				declarationPreview,
			...
			if (totalMatches > shownMatches)
				journal?.RecordNotice(AgentJournalNoticeCodes.MatchesOmitted);
	...
	[Description(
		"Finds evidenced dependencies and dependents for one to 16 seed files without widening selection. Use it after search_project or get_tree; use search_project for text references or get_file for content. Returns paths, evidence, resolution status, token estimates, language coverage, partial-parse and configuration diagnostics, and scope; results above 50,000 characters use read_pack. Key parameters: path, direction=dependencies|dependents|both, profile, git_scope, include_patterns, exclude_patterns, and max_file_bytes.")]
	...
	private async Task<(DependencyRelatedResult Result, int UninspectedSources)> ProtectRelatedEvidenceAsync(
		...
		CancellationToken cancellationToken)
	{
		var displayedReasons = related.Seeds
			.SelectMany(static seed => seed.Dependencies.Concat(seed.Dependents))
			.SelectMany(static file => file.Reasons)
			.ToHashSet(StringComparer.Ordinal);
		var evidence = related.Index.Edges
			.SelectMany(edge => edge.Evidence.Select(site => new RelatedEvidence(
				edge,
				site,
				FormatRelatedEvidenceReason(edge, site))))
		...
		IReadOnlyList<RelatedFile> Protect(IReadOnlyList<RelatedFile> files) => files
			.Select(file => file with
			{
				Reasons = file.Reasons
					.Select(reason => replacements.GetValueOrDefault(reason, reason))
					.Distinct(StringComparer.Ordinal)
					.Order(StringComparer.Ordinal)
					.ToArray()
			})
	...
	private static string RelatedEvidenceLabel(EvidenceLayer layer) => layer switch
	{
	...
	[Description(
		"Reads selected text after mandatory secret and configured private-data replacement. Use it after get_tree or search_project; use pack_context for broad multi-file context. Use path for one page or requests for up to eight files and sixteen file/range/symbol selections; forms are exclusive. Send one batched call whenever you want more than one file, range, or known symbol: requests=[{\"path\":\"src/a.ts\",\"symbol\":\"Router.load\"}]. Items report ok, partial, not-returned, or unavailable; overlapping ranges share a 1,000-line/50,000-character cap. Coordinates use returned text; start_column continues a page.")]
	public Task<CallToolResult> GetFile(
		...
		RunProjectAsync("get_file", request, async () =>
		{
			...
			journal?.RecordProtection(
				CountRedactionsOnLines(transformed, Enumerable.Range(page.StartLine, page.EndLine - page.StartLine + 1)),
				inspected.Snapshot);
			var characterLimitNotice = page.CharacterLimitReached
				? "[The current line exceeded the 50000-character response cap; use search_project to narrow the source.]"
				: null;
		...
		CancellationToken cancellationToken)
	{
		...
		for (var index = 0; index < resolvedRequests.Count; index++)
		{
			...
			var located = McpSearchSymbols.ResolveSymbol(
				...
				cancellationToken);
			if (located.Status != McpSymbolLookupStatus.Resolved)
			{
				resolvedRequests[index] = resolved with
				{
					UnavailableReason = FormatBatchSymbolFailure(located)
				};
				continue;
			}
			resolvedRequests[index] = resolved with
		...
		if (rendered.UnavailableNotice is not null)
			journal?.RecordNotice(AgentJournalNoticeCodes.Unavailable);
		if (resolvedRequests.Any(static item => item.UnavailableReason == McpErrorCodes.PathNotFound))
			journal?.RecordNotice(AgentJournalNoticeCodes.MissingPath);
		return McpToolResults.TextSuccess(AppendTrustedNotices(
			spotlighted,
			rendered.Summary,
		...
		McpSearchDeclarationPreview? declarationPreview)
	{
		if (journal is null)
			return;
		var returnedLines = rendered.RenderedLines
			.GroupBy(static line => line.RelativePath, StringComparer.Ordinal)
			.ToDictionary(
				static group => group.Key,
				static group => group.Select(static line => line.LineNumber).ToHashSet(),
				StringComparer.Ordinal);
		if (declarationPreview is { IsAddressable: true, Text.Length: > 0 } preview)
		{
			if (!returnedLines.TryGetValue(preview.Declaration.RelativePath, out var lines))
			{
				lines = [];
				returnedLines[preview.Declaration.RelativePath] = lines;
			}
			var displayedEnd = Math.Max(
				preview.Declaration.StartLine,
				preview.Declaration.EndLine - preview.RemainingLines);
			for (var line = preview.Declaration.StartLine; line <= displayedEnd; line++)
				lines.Add(line);
		}
	...
	private static IReadOnlyDictionary<int, int> BuildRedactionLineCounts(TransformedTextFile file)
	{
		...
		{
			var target = Math.Clamp(range.Start, sourceIndex, file.Content.Length);
			while (sourceIndex < target)
			{
				var character = file.Content[sourceIndex++];
	...
	private static (long Secrets, long PrivateData) ResolveProtectionCounts(
		...
		SecretRedactionSnapshot? snapshot)
	{
		if (snapshot is null || replacementCount <= 0)
			return (0, 0);
		...
		if (secrets == 0)
			return (0, replacementCount);
		return replacementCount == secrets + privateData
			? (secrets, privateData)
			: (0, 0);
	}
		...
		CancellationToken cancellationToken)
	{
		...
		var deliveredRanges = new List<McpDeliveredFileRange>();
		var usedLines = 0;
		...
		IReadOnlyList<McpResolvedFileReadRequest> requests)
	{
		...
		foreach (var request in requests)
		{
			if (request.UnavailableReason is not null)
				continue;
			foreach (var range in request.Request.Ranges)
			{
				var matching = groups
	...
	private sealed record McpBatchFileReadResult(
		...
		IReadOnlyList<McpDeliveredFileRange> DeliveredRanges);

	private readonly record struct McpDeliveredFileRange(
		string? PhysicalPath,
		int StartLine,
		int EndLine);
	...
	private sealed record RelatedEvidence(DependencyEdge Edge, SourceSite Site, string Reason);
	...
	private static string? FormatDetailMix(ContentDetailMix? mix)
	{
		...
		var details = $"[Detail] unmatched: {names}" +
			(remaining > 0
				? $" and {remaining.ToString(CultureInfo.InvariantCulture)} more"
				: string.Empty);
		return summary + "\n" + McpSpotlight.Wrap(details);
	}
		...
		McpDeclarationReadContext? readContext = null)
	{
		...
		var body = preview is { IsAddressable: true, Text.Length: > 0 }
			? FormatDeclarationBody(preview, declarations.Count, readContext)
			: string.Empty;
	...
	private static string FormatDeclarationBody(
		...
		McpDeclarationReadContext? readContext)
	{
		var declaration = preview.Declaration;
		var arguments = FormatDeclarationReadArguments(declaration, readContext);
		var output = new StringBuilder()
			.AppendLine()
	...
	private McpDeclarationReadContext? CreateDeclarationReadContext(ProjectContextPlan plan)
	{
		var remote = plan.SourceIdentity is { SourceType: ProjectSourceType.GitClone };
		...
		McpDeclarationReadContext? context)
	{
		...
		if (context?.Project is { Length: > 0 } project)
