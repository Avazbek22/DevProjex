...
public sealed class SecretRedactionSession : IDisposable
{
	...
	private const int CandidateMetadataBaseRetainedBytes = 120;
	private const int OccurrenceIdRetainedBytes = 24 + 64 * sizeof(char);

	private readonly ISecretDetector _detector;
	private readonly ISecretDetector? _privateDataDetector;
	...
	private Task? _privateDataDetectorWarmUpTask;
	...
	public SecretRedactionSession(
		...
		: this(detector, new SecretScanCache(), persistentMarkStore, persistentIdentityProvider, null)
	{
	}

	public static SecretRedactionSession CreateWithPrivateData(
		ISecretDetector detector,
		ISecretDetector privateDataDetector,
		IPersistentSecretMarkStore? persistentMarkStore = null,
		IPersistentSecretIdentityProvider? persistentIdentityProvider = null) =>
		new(
			detector,
	...
	internal SecretRedactionSession(
		...
		IPersistentSecretMarkStore? persistentMarkStore = null,
		IPersistentSecretIdentityProvider? persistentIdentityProvider = null)
		: this(detector, scanCache, persistentMarkStore, persistentIdentityProvider, null)
	{
	}

	private SecretRedactionSession(
		ISecretDetector detector,
		SecretScanCache scanCache,
		IPersistentSecretMarkStore? persistentMarkStore,
		IPersistentSecretIdentityProvider? persistentIdentityProvider,
		ISecretDetector? privateDataDetector)
	{
		_detector = detector ?? throw new ArgumentNullException(nameof(detector));
		_privateDataDetector = privateDataDetector;
	...
	/// </summary>
	public Task BeginWarmUp() => BeginWarmUp(SecretRedactionFeatures.Secrets);

	public Task BeginWarmUp(SecretRedactionFeatures features)
	{
		ValidateFeatures(features);
		lock (_sync)
		{
			ObjectDisposedException.ThrowIf(_disposed, this);
			Task? secretsTask = null;
			Task? privateDataTask = null;
			if ((features & SecretRedactionFeatures.Secrets) != 0)
				secretsTask = EnsureDetectorWarmUpLocked(_detector, ref _detectorWarmUpTask);
			if ((features & SecretRedactionFeatures.PrivateData) != 0)
			{
				privateDataTask = EnsureDetectorWarmUpLocked(
					_privateDataDetector!,
					ref _privateDataDetectorWarmUpTask);
			}
			return secretsTask is null
				? privateDataTask!
				: privateDataTask is null
					? secretsTask
					: Task.WhenAll(secretsTask, privateDataTask);
		}
	}
	...
	internal Task EnsureWarmUpAsync(
		SecretRedactionFeatures features,
		CancellationToken cancellationToken) =>
		BeginWarmUp(features).WaitAsync(cancellationToken);
		...
		SecretRedactionFeatures features = SecretRedactionFeatures.Secrets)
	{
		...
		return new SecretRedactionScope(
			this,
			normalizedProjectRoot,
			selection.CreateTransformFingerprint(GetSnapshotTransformIdentity(transformIdentity, features)),
			keptOccurrences,
			overrideRevision,
			snapshotRevision,
	...
	public int? GetRedactionCount(
		...
		string transformIdentity = "",
		SecretRedactionFeatures features = SecretRedactionFeatures.Secrets)
		=> GetSnapshot(projectRoot, orderedFilePaths, transformIdentity, features)?.RedactedCount;
	...
	{
		ValidateFeatures(features);
		var key = GetOrComputeSelectionKey(
			projectRoot,
			orderedFilePaths,
			GetSnapshotTransformIdentity(transformIdentity, features));
		lock (_sync)
		{
		...
		bool allowIdentityTransformFallback = false)
	{
		...
		var resolved = SecretRedactionScope.ResolveSegmentedFindings(rawFindings);
		var candidates = new SecretFindingCandidateMetadata[resolved.Candidates.Count];
		for (var index = 0; index < resolved.Candidates.Count; index++)
		{
			cancellationToken.ThrowIfCancellationRequested();
			generationToken.ThrowIfCancellationRequested();
			var finding = resolved.Candidates[index];
			var valueFingerprint = HashValue(content.Slice(finding.Start, finding.Length));
			var occurrenceCoordinateIdentity = ResolveOccurrenceCoordinateIdentity(
				finding.Start,
				finding.Length,
				transformMap);
			candidates[index] = new SecretFindingCandidateMetadata(
				finding.Start,
				finding.Length,
				finding.RuleId,
				valueFingerprint,
				...
				finding.Category,
				occurrenceCoordinateIdentity);
		}
		var segments = resolved.Segments
			.Select(static segment => new SecretFindingSegmentMetadata(
				segment.Start,
				segment.Length,
				segment.CandidateIndexes))
			.ToArray();
		...
		CancellationToken generationToken)
	{
		...
		var entry = new SecretScanCacheEntry(
			...
			Candidates: [],
			Segments: [],
			ApproximateRetainedBytes: 112 +
			                          (normalizedPath.Length + rulesIdentity.Length + transformIdentity.Length +
			                           projectRoot.Length + relativePath.Length) * sizeof(char));
		lock (_sync)
		{
			ThrowIfGenerationIsNotCurrentLocked(generation, generationToken);
	...
	internal ISecretDetectionScope CreateDetectorScope(
		string projectRoot,
		SecretRedactionFeatures features)
	{
		ValidateFeatures(features);
		var secretsScope = (features & SecretRedactionFeatures.Secrets) != 0
			? _detector.CreateScope(projectRoot)
			: null;
		var privateDataScope = (features & SecretRedactionFeatures.PrivateData) != 0
			? _privateDataDetector!.CreateScope(projectRoot)
			: null;
		return secretsScope is null
			? privateDataScope!
			: privateDataScope is null
				? secretsScope
				: new CombinedSecretDetectionScope(secretsScope, privateDataScope);
	}
	...
	internal static string BuildSelectionKey(
		...
		ContentSelectionSnapshot
			...
			.CreateTransformFingerprint(GetSnapshotTransformIdentity(transformIdentity, features));

	private static string GetSnapshotTransformIdentity(
		string transformIdentity,
		SecretRedactionFeatures features) =>
		features switch
		{
			SecretRedactionFeatures.Secrets => transformIdentity,
			SecretRedactionFeatures.PrivateData => $"{transformIdentity}\nredaction:private-data",
			SecretRedactionFeatures.Secrets | SecretRedactionFeatures.PrivateData =>
				$"{transformIdentity}\nredaction:secrets+private-data",
			_ => throw new ArgumentOutOfRangeException(nameof(features), features, null)
		};

	private void ValidateFeatures(SecretRedactionFeatures features)
	{
		const SecretRedactionFeatures supported =
			SecretRedactionFeatures.Secrets | SecretRedactionFeatures.PrivateData;
		if (features == SecretRedactionFeatures.None || (features & ~supported) != 0)
			throw new ArgumentOutOfRangeException(nameof(features), features, null);
		if ((features & SecretRedactionFeatures.PrivateData) != 0 && _privateDataDetector is null)
		{
	...
	internal static SecretOccurrenceCoordinateIdentity ResolveOccurrenceCoordinateIdentity(
		int start,
		int length,
		ContentTransformMap? transformMap)
	{
		if (transformMap is null or { IsIdentity: true })
			return new SecretOccurrenceCoordinateIdentity(IsSourceBacked: true, start, length);
		...
		{
			return new SecretOccurrenceCoordinateIdentity(
				IsSourceBacked: true,
				sourceStart,
				sourceLength);
		}

		return new SecretOccurrenceCoordinateIdentity(IsSourceBacked: false, start, length);
	}
		...
		IReadOnlyList<SecretFindingSegmentMetadata> segments)
	{
		long bytes = 160 +
		             (normalizedPath.Length + contentFingerprint.Length + rulesIdentity.Length +
		              transformIdentity.Length + occurrenceProjectRoot.Length + occurrenceRelativePath.Length) *
		             sizeof(char);
		foreach (var candidate in candidates)
		{
			// The 64-character occurrence hash is materialized lazily. Reserving it up front keeps
			// the cache byte limit valid after the first preview without a mutable accounting path.
			bytes += CandidateMetadataBaseRetainedBytes + OccurrenceIdRetainedBytes +
			         (candidate.RuleId.Length + candidate.ValueFingerprint.Length) * sizeof(char);
			if (candidate.PersistentMarkId is { RelativePath: { } relativePath })
				bytes += relativePath.Length * sizeof(char);
		}
		foreach (var segment in segments)
			bytes += 40 + segment.CandidateIndexes.Count * sizeof(int);
		return bytes;
	}
...
public sealed class SecretRedactionScope
{
	...
	private int _privateDataDetectedCount;
	private int _privateDataRedactedCount;
		...
		SecretRedactionFeatures features = SecretRedactionFeatures.Secrets)
	{
		...
		_detectorScope = session.CreateDetectorScope(_projectRoot, features);
	...
	internal void ProcessEntry(string filePath, SecretScanCacheEntry entry)
	{
		...
		try
		{
			...
			if (entry.IsUnscannable)
				RecordUnscannable(filePath);
			AccumulateFindings(filePath, entry, transformMap: null);
		}
		finally
		{
			ExitOrderedConsumer();
		...
		CancellationToken cancellationToken = default)
	{
		var entry = DetectTransformed(
			...
			cancellationToken);
		if (entry is null)
			return ProcessFindings(filePath, entry: null, transformMap: transformMap);
		...
		CancellationToken cancellationToken = default)
	{
		...
		if (entry is null)
		{
			AccumulateFindings(filePath, entry: null, transformMap: transformMap);
			return;
		}
		...
		for (var index = 0; index < segments.Count; index++)
		{
			...
			var replacement = kept
				...
				: SecretRedactionLegend.CreatePlaceholder(
					...
					identityIndexes[displayCandidateIndex]);
			var outputStart = checked(segment.Start + outputDelta);
			var outputLength = replacement?.Length ?? segment.Length;
			replacements[index] = new SecretReplacement(
				segment.Start,
				segment.Length,
				replacement);
			IReadOnlyList<string>? cascadedOccurrenceIds = null;
			if (kept && segment.CandidateIndexes.Count > 1)
			{
				var ids = new string[segment.CandidateIndexes.Count];
				for (var candidateIndex = 0; candidateIndex < ids.Length; candidateIndex++)
					ids[candidateIndex] = occurrenceIds[segment.CandidateIndexes[candidateIndex]];
				cascadedOccurrenceIds = ids;
			}
			spans[index] = new SecretPreviewSpan(
				occurrenceIds[displayCandidateIndex],
				displayCandidate.RuleId,
				outputStart,
				outputLength,
		...
		for (var index = 0; index < candidates.Count; index++)
		{
			...
			if ((state & CandidateRedacted) != 0)
			{
				...
				if (candidate.Category == RedactionFindingCategory.PrivateData)
					_privateDataRedactedCount++;
				redactedInFile++;
				if (candidate.PersistentMarkHash is { Length: > 0 } markHash)
		...
		ContentTransformMap? transformMap)
	{
		...
		var namespaceMatches = entry is not null &&
		                       string.Equals(
			                       _projectRoot,
			                       entry.OccurrenceProjectRoot,
			                       StringComparison.Ordinal) &&
		                       string.Equals(
			                       relativePath,
			                       entry.OccurrenceRelativePath,
			                       StringComparison.Ordinal);
		for (var index = 0; index < candidates.Count; index++)
		{
			var candidate = candidates[index];
			var coordinateIdentity = transformMap is null
				? candidate.OccurrenceCoordinateIdentity
				: SecretRedactionSession.ResolveOccurrenceCoordinateIdentity(
					candidate.RawStart,
					candidate.RawLength,
					transformMap);
			var cachedOccurrenceId = namespaceMatches &&
			                         coordinateIdentity == candidate.OccurrenceCoordinateIdentity
				? candidate.GetCachedOccurrenceId()
				: null;
			if (cachedOccurrenceId is not null)
			{
				occurrenceIds[index] = cachedOccurrenceId;
				continue;
			}
	...
	{
		var resolvedSet = ResolveSegmentedFindings(matches);
		if (resolvedSet.Segments.Count == 0)
			return [];
		var resolved = new DetectedSecret[resolvedSet.Segments.Count];
		for (var index = 0; index < resolvedSet.Segments.Count; index++)
		{
			var segment = resolvedSet.Segments[index];
			var winner = resolvedSet.Candidates[segment.CandidateIndexes[0]];
			var valueOffset = segment.Start - winner.Start;
			resolved[index] = winner with
			{
				Start = segment.Start,
				Length = segment.Length,
				Value = winner.Value.Substring(valueOffset, segment.Length)
			};
		}
		return resolved;
	}
	...
	{
		var resolvedSet = ResolveSegmentedFindings(matches);
		return resolvedSet.Segments
			.Select(segment => new ResolvedSecretFinding(
				segment.Start,
				segment.Length,
				segment.CandidateIndexes.Select(index => resolvedSet.Candidates[index]).ToArray()))
			.ToArray();
	}

	internal static ResolvedSecretFindingSet ResolveSegmentedFindings(
		IReadOnlyList<DetectedSecret> matches)
	{
		if (matches.Count == 0)
			return ResolvedSecretFindingSet.Empty;

		var merged = matches
			.GroupBy(static match => (match.Start, match.Length, match.Category))
		...
		for (var index = 0; index < candidates.Length; index++)
		{
			...
			boundaries[index * 2] = candidate.Start;
			boundaries[index * 2 + 1] = end;
			AddBoundaryEvent(starts, candidate.Start, index);
			AddBoundaryEvent(ends, end, index);
		}
		Array.Sort(boundaries);

		var active = new SortedSet<int>();
		var segments = new List<ResolvedSecretSegment>(Math.Max(1, candidates.Length * 2 - 1));
		var boundaryIndex = 0;
		while (boundaryIndex < boundaries.Length)
		{
			...
			{
				segments.Add(new ResolvedSecretSegment(
					boundary,
		...
		return new ResolvedSecretFindingSet(candidates, segments);
	}

	private static void AddBoundaryEvent(
		IDictionary<int, List<int>> events,
		int position,
		int candidateIndex)
	{
		if (!events.TryGetValue(position, out var candidates))
		{
			candidates = [];
			events.Add(position, candidates);
		}
	...
	internal sealed record ResolvedSecretSegment(
		int Start,
		int Length,
		IReadOnlyList<int> CandidateIndexes);

	internal sealed record ResolvedSecretFindingSet(
		IReadOnlyList<DetectedSecret> Candidates,
		IReadOnlyList<ResolvedSecretSegment> Segments)
	{
