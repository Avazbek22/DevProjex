...
public sealed class MainWindowWorkspaceInteractionUiTests(UiWorkspaceFixture workspace)
{
	...
	[AvaloniaFact]
	public async Task ProjectProfiles_AreFlushedBeforeProjectSwitch_WhenApplySaveFails()
	{
		...
		try
		{
			...
			var profileCoordinator = GetRequiredPrivateField<ProjectProfilePersistenceCoordinator>(
				window,
				"_projectProfiles");
			Assert.True(profileCoordinator.HasPendingWrites);
			await SaveUi5ArtifactAsync(
				window,
				"issue-03-profile-pending-before-switch",
				$"saveAttempts={flakyStore.SaveAttemptCount}",
				$"pendingWrites={profileCoordinator.HasPendingWrites}",
				$"sourceProject={workspace.Project.RootPath}");

			await UiTestDriver.OpenFolderAsync(window, secondProject.RootPath);

			Assert.True(flakyStore.SaveAttemptCount >= 2);
			Assert.True(flakyStore.TryLoadProfile(workspace.Project.RootPath, out var switchedProfile));
			Assert.NotNull(switchedProfile);
			await SaveUi5ArtifactAsync(
				window,
				"issue-03-profile-flushed-after-switch",
				$"saveAttempts={flakyStore.SaveAttemptCount}",
				$"pendingWrites={profileCoordinator.HasPendingWrites}",
				$"sourceProfileSaved={switchedProfile is not null}",
				$"targetProject={secondProject.RootPath}");
	...
	[AvaloniaFact]
	public async Task PendingApplyFlushPreservesNewerTreeSelectionBeforeProjectSwitch()
	{
		using var secondProject = UiTestProject.CreateDefault();
		var appDataPath = Path.Combine(workspace.Project.AppDataPath, Guid.NewGuid().ToString("N"));
		Directory.CreateDirectory(appDataPath);
		...
		try
		{
			...
			await UiTestDriver.WaitForConditionAsync(
				...
				"project to finish loading before the pending Apply write");
			await UiTestDriver.WaitForConditionAsync(
				window,
				() => UiTestDriver.GetBoundsInWindow(
					UiTestDriver.GetRequiredControl<Border>(window, "SettingsContainer"),
					window).Width >= 200,
				"settings pane to become available before the pending Apply write");

			var lockPath = store.GetPath() + ".lock";
			Directory.CreateDirectory(Path.GetDirectoryName(lockPath)!);
			var markdownWasSelected = UiTestDriver.GetViewModel(window).Extensions
				.Single(static option => string.Equals(option.Name, ".md", StringComparison.OrdinalIgnoreCase))
				.IsChecked;
							 ...
							 FileShare.None))
			{
				...
				await UiTestDriver.RaiseButtonClickAsync(
					UiTestDriver.GetRequiredApplySettingsButton(window));
				await UiTestDriver.WaitForConditionAsync(
					window,
					() =>
						!UiTestDriver.GetViewModel(window).StatusBusy &&
						GetRequiredPrivateField<ProjectProfilePersistenceCoordinator>(
							window,
							"_projectProfiles").HasPendingWrites,
					"failed Apply profile write to become pending");
			}
			...
			await UiTestDriver.WaitForConditionAsync(
				...
				() => GetRequiredPrivateField<TreeSelectionProfilePersistenceCoordinator>(
					window,
					"_treeSelectionProfiles").State.Phase == SelectionPersistencePhase.Pending,
				"newer tree selection frontier to become pending");
			...
			var persisted = store.LookupProfile(
				...
				TimeSpan.FromSeconds(1));
			Assert.Equal(ProjectProfileLookupStatus.Found, persisted.Status);
			Assert.Equal(["src"], persisted.Profile!.SelectedPaths);
			Assert.Equal(
				!markdownWasSelected,
				persisted.Profile.ExtensionStates![".md"]);
		}
		finally
		{
			if (window.IsVisible)
