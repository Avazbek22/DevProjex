...
public sealed partial class McpServerIntegrationTests
{
	...
	[Fact(Timeout = 60_000)]
	public async Task ReadPackDoesNotChangeTheRevisionOfAnInFlightLiveSearch()
	{
		using var workspace = new TemporaryDirectory();
		var project = workspace.CreateDirectory("project");
		var oldFolder = Path.Combine(project, "old");
		...
		Directory.CreateDirectory(newFolder);
		File.WriteAllText(
			Path.Combine(oldFolder, "Old.cs"),
			"class Old { string marker = \"needle\"; }\n" + new string('x', 60_000) + "\n");
		File.WriteAllText(Path.Combine(newFolder, "New.cs"), "class New {}\n");
		var profiles = new ProjectProfileStore(() => Path.Combine(workspace.Path, "app-data"));
		profiles.SaveProfile(project, new ProjectSelectionProfile([], [], [], SelectedPaths: ["old"]));
		await using var server = await McpTestServer.StartAsync(project, workspace.Path, live: true);
		var packed = await server.CallAsync("pack_context", new Dictionary<string, object?>
		{
			["view"] = "content",
			["format"] = "text"
		});
		Assert.NotEqual(true, packed.IsError);
		var packId = ExtractPackId(AllText(packed));
		var scanEntered = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
		using var releaseScan = new ManualResetEventSlim();
		McpSearchExecutionHooks.AfterScan = path =>
		{
		...
		try
		{
			...
			await scanEntered.Task.WaitAsync(TimeSpan.FromSeconds(10), TestContext.Current.CancellationToken);
			profiles.SaveProfile(project, new ProjectSelectionProfile([], [], [], SelectedPaths: ["new"]));
			using var canceledReadToken = CancellationTokenSource.CreateLinkedTokenSource(
				TestContext.Current.CancellationToken);
			var canceledRead = server.Client.CallToolAsync(
				"read_pack",
				new Dictionary<string, object?> { ["pack_id"] = packId },
				cancellationToken: canceledReadToken.Token).AsTask();
			Assert.NotSame(
				canceledRead,
				await Task.WhenAny(
					canceledRead,
					Task.Delay(TimeSpan.FromMilliseconds(100), TestContext.Current.CancellationToken)));
			canceledReadToken.Cancel();
			await Assert.ThrowsAnyAsync<OperationCanceledException>(() => canceledRead);

			var read = server.CallAsync("read_pack", new Dictionary<string, object?> { ["pack_id"] = packId });
			var readCompletedDuringSearch = await Task.WhenAny(
				read,
				Task.Delay(TimeSpan.FromSeconds(2), TestContext.Current.CancellationToken)) == read;
			releaseScan.Set();
