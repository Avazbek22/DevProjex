...
public sealed class SecretRedactionOutputContractIntegrationTests
{
	...
	[InlineData(ProjectContextDocumentFormat.Json, true)]
	[InlineData(ProjectContextDocumentFormat.Xml, true)]
	public async Task CompleteContext_LocalUserPathFollowsHidePrivateDataSetting(
		ProjectContextDocumentFormat format,
		bool hidePrivateData)
	{
		using var temporary = new TemporaryDirectory();
		var projectRoot = temporary.CreateDirectory("complete-private-path-project");
		var plan = await BuildPlanAsync(projectRoot, hideSecrets: false, hidePrivateData);
		var privateRoot = OperatingSystem.IsWindows()
			? @"C:\Users\private-path-owner\project"
			: "/home/private-path-owner/project";
		...
		await using var prepared = await new SecretRedactionOutputPreparer(new FileContentAnalyzer())
			.PrepareAsync(
				...
				[],
				TestContext.Current.CancellationToken);
		await using var preparedDestination = new MemoryStream();
		await service.WritePreparedCompleteAsync(
			...
			TestContext.Current.CancellationToken,
			useSourceMappedStructuredPaths: true);
		var preparedDocument = Encoding.UTF8.GetString(preparedDestination.ToArray());
		Assert.Contains(OutputRootPathPresentation.LocalUserPlaceholder, preparedDocument, StringComparison.Ordinal);
		Assert.DoesNotContain("private-path-owner", preparedDocument, StringComparison.Ordinal);
	}
