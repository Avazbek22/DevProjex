...
public sealed class CommandExecutionTests
{
	...
	[Fact]
	public async Task WrappedMacOsTerminalRestoreFailureUsesStableRuntimeContract()
	{
		const string rawAggregateMessage = "RAW_TERMINAL_TASK_FAILURE";
		var environment = new TestTerminalEnvironment();

		var exitCode = await CommandExecution.RunAsync(
			environment,
			new TerminalOutputOptions(
				Verbosity: TerminalVerbosity.Diagnostic),
			() => throw new AggregateException(
				rawAggregateMessage,
				new AggregateException(
					new MacOsTerminalRestoreException(nativeError: 9))));
		...
		Assert.DoesNotContain(
			...
			StringComparison.Ordinal);
		Assert.Contains(
			typeof(MacOsTerminalRestoreException).FullName!,
			environment.StandardError,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
	...
	[Fact]
	public async Task DirectMacOsTerminalRestoreFailureIsNotReportedAsFilesystemFailure()
	{
		var environment = new TestTerminalEnvironment();
		...
		var exitCode = await CommandExecution.RunAsync(
			environment,
			new TerminalOutputOptions(),
			() => throw new MacOsTerminalRestoreException(nativeError: 9));
	...
	[Fact]
	public async Task MixedTerminalRestoreAggregateRemainsUnexpected()
	{
		const string unrelatedFailure = "UNRELATED_FAILURE_SENTINEL";
		var environment = new TestTerminalEnvironment();
		...
		var exitCode = await CommandExecution.RunAsync(
			...
			() => throw new AggregateException(
				new MacOsTerminalRestoreException(nativeError: 9),
				new InvalidOperationException(unrelatedFailure)));
	...
	[Fact]
	public async Task UnrelatedSingletonAggregateRemainsUnexpected()
	{
		const string unrelatedFailure = "UNRELATED_SINGLE_FAILURE_SENTINEL";
		var environment = new TestTerminalEnvironment();
