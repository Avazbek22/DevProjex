...
public sealed class ReportPathResolver(
	...
	Func<Guid>? reportIdProvider = null)
{
	...
	public string Resolve(string? reportPath = null)
	{
		...
		var reportId = _reportIdProvider().ToString("N");
		return Path.Combine(baseDirectory, AppFolderName, ReportsFolderName, $"devprojex-report-{timestamp}-{reportId}.json");
	}
