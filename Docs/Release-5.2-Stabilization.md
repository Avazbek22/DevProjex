...
| GUI folder-open preflight ordering | `ac4ea3ca` | A slow root-access probe for older folder A could finish after newer folder B was already opened, then enter the load pipeline as its newest request and replace B. Entry order is now retained through successful preflight and pending-selection decisions before source metadata or the load pipeline is touched. The deterministic headless race failed before the fix; afterward both new scenarios passed, including a newer failed-preflight control that still lets A open, plus five adjacent UI lifecycle/open checks. |
| Token-limited transformed export admission | `f5fe579f` | A source classified unscannable during measurement was admitted at zero cost, then could recover during materialization and emit full content despite `--max-tokens 1`. The shared measured-admission path now omits such unmeasured sources from the post-admission plan while retaining their trusted omission notice. CLI also merges admission-stage and write-stage omissions for stdout and file destinations, preferring the actual write classification on overlap. Deterministic red cases exposed the budget breach and missing second-pass warning; four new and four adjacent Release Terminal checks passed afterward. Six focused MCP pack/token-budget Integration checks passed after the shared change. |
| Shared dependency-file wait cancellation | `e839a373` | A canceled waiter joined to another request's blocked file extraction previously remained suspended until the owner completed. The joined task is now waited with the caller's token; caller cancellation does not evict the healthy owner's cache entry. The deterministic blocked-owner regression failed before the fix and passed afterward, with seven focused Release Unit checks covering owner-cancel retry, eviction generation, and warm reuse. The independent owner still completes and the file is parsed once. |
...
| Stale tree-refresh diagnostics | `ab05d093` | A superseded F5/filter tree build could show obsolete Git-scope or root-access diagnostics before the existing freshness check suppressed publication. Input freshness is now checked immediately after the build and before those handlers, while the later publication guard remains. Two deterministic stale scenarios failed before the fix; six focused Release Unit cases passed afterward, including current-request diagnostic controls. |
| Protected binary-source replacement | `a632c2fa` | A `.txt` classified as binary during Hide Secrets preparation could be replaced with secret-bearing text before the folder or ZIP copier opened it; the prepared binary record had discarded its inspected source metadata, so the normal source-version guard was a no-op. The record now retains that metadata and the existing guard rejects a changed source before copy. Both folder and ZIP regressions failed before the fix and six focused/adjacent Release Integration cases passed afterward. The freshness contract compares length and modification time, not adversarial same-size/restored-time replacements. |
...
| Disappearing source in transformed search | `fe8c0b52` | A selected file removed after `search_project` planning could throw `FileNotFoundException` from the ordered redaction producer and abort the entire result, discarding matches already observed. Consumer-mode preparation now records only a verified missing source as an unscannable omission, without a prepared entry, content callback, or prepared-file I/O; producer/worker leases and ordering remain bounded. A deterministic nine-file red regression failed with a tool error before the fix and returned a trusted partial `8/9` result afterward. Seven focused/adjacent Release Integration checks passed, including the unchanged strict `PrepareAsync` disappearance error and explicit `get_file` withholding. |
| Git case-semantics refresh regression | `a450da31` | A production-path headless test changed `core.ignoreCase` while the project remained open, then used F5 and confirmed the GUI tree followed the new `.gitignore` semantics. The test passed before any proposed fix: the single-entry rule-build cache naturally misses during the multi-phase refresh and rebuilds against current Git semantics. No speculative invalidation or F5 performance regression was added. Release UI 1/1 passed. |
| Obsolete theme primary with current backup | `4b46ee70` | Startup previously reset an obsolete primary theme document before trying its valid current backup, replacing a customized preset with factory defaults. It now tries the backup first and retains the existing reset only when recovery is unavailable. The red theory exposed one failing startup case while `EnsureStorageExists` was already correct; afterward 2/2 focused and 43/43 adjacent Release Unit checks passed. |
| Stale GUI project-copy picker | `798d9894` | A folder/ZIP export started for project A could await confirmation or the native picker while project B opened, then export B to A's chosen destination because the request used mutable current state. Handlers now capture the source path/tree epoch before awaiting and recheck it after each await and immediately before export; late errors are suppressed for superseded or closed windows. The headless red case physically created B's copy before the fix. The affected UI class passed 5/5 after the fix and again after the error-guard refinement. No animation delay changed. |
| Truncated stored MCP pack | `8f090c4a` | `read_pack` verified the private pack file's handle identity but not its expected byte length. Truncating the same file in place could therefore return a successful altered or short page against stale checkpoints. The opened handle now verifies stored length and maps mismatch to `DPX-MCP-PACK-EXPIRED` through existing reader cleanup. The regression failed before the fix by returning altered content; afterward three focused Integration and 30 adjacent Unit checks passed, with one Windows file-alias capability skip. Same-length mutation or a write after the open-time check is not covered by this narrow guard. |
| Redundant TUI selection revision | `48cde853` | A repeated `select` command that changed no selection still incremented the state revision while scheduling no replacement projection. It could invalidate the first command's pending 180 ms reprojection, leaving plan, metrics, and preview stale until another real selection change. Revision now advances only for an effective selection or frontier-intent transition. The deterministic pending-projection regression failed before the fix (revision 3 instead of 2); 34 focused Release Terminal state checks passed afterward. Fixed scheduling delay is unchanged. |
| Invalid Git branch lease | `32dafb0a` | An invalid requested branch could throw during cached Git-session acquisition after taking a repository lease, leaving the lease held until service disposal. Branch validation now precedes lease acquisition. The focused red test observed the exclusive lease unavailable after the exception; three Release Unit isolation checks passed afterward, covering invalid, valid, and canceled requests. |
| Late Git branch catalog after project switch | `742fe34e` | An asynchronous branch-list response for cached Git project A could arrive after local project B opened and repopulate B's branch menu. The response now checks cancellation, Git mode, and the current repository path before publication. A deterministic headless race failed before the fix; the new and adjacent Release UI tests passed 2/2 afterward. No animation delay changed. |
| Newly unchecked profile options | `17562888` | A merge based on an older profile treated a newly explicit `false` extension, root-folder, or ignore-option state as an effective `false` to `false` no-op and silently omitted it, despite reporting a successful save. New explicit unchecked keys now count as deltas only when their state field is allowed, retaining passive true materialization and unrelated concurrent fields. Three regressions failed before the fix; 10 focused profile and 71 adjacent persistence Release Unit tests passed afterward. |
| F5 profile crossing a project switch | `36e7f765` | An F5 refresh awaiting project A's profile lookup could resume after B opened and apply A's options to B; the red headless test observed B's `StripBlankLines` switch from false to true while B's tree remained current. An accepted folder open now cancels the older refresh; reload captures one project path and rechecks it after awaits; stale completion and error toasts are suppressed. The A-to-B and same-path-reopen regressions passed 2/2, with three adjacent Release UI checks passing. Fixed animation delays are unchanged. |
