...
| `mcp connect` | `[PROJECT]`, `--client`, `--mode` | current directory, `claude-code`, `live` | prints one connection fragment using the absolute installed CLI path | clients are `claude-code`, `codex`, `json`; modes are `live`, `standard`; it starts no server | stdout is only the generated fragment; invalid choices exit `2` | parser, path resolver, fragment generator, process |
