...
| `mcp` | `--live` | off | rereads the local project profile before each tool call and uses its saved tree frontier and filters as the server baseline | startup-only; `profile` may be absent or `local`; another profile source is rejected; explicit call paths and globs only narrow the saved focus; named `get_file` reads may cross the focus but never the effective filters | every tool response adds trusted revision state; stored packs retain their build revision | parser, profile store, MCP process |
| `mcp connect` | `[PROJECT]`, `--client`, `--mode`, `--print`, `--open` | current directory, `claude-code`, `live`, off, off | connects the selected client using the absolute installed CLI path; `--print` only generates its configuration; `--open` opens the client after registration | clients are `claude-code`, `codex`, `cursor`, `vscode`, `json`; modes are `live`, `standard`; Claude Code is project-local, Codex configuration is global, Cursor writes `.cursor/mcp.json`, VS Code writes `.vscode/mcp.json`, and `json` is manual-only; `--print` performs no client process or project write; `--print` and `--open` conflict; `--open` with `json` is rejected | default stdout describes the connection result before the open result; `--print` stdout is only the fragment; `--open` failure after successful registration reports the manual launch command and exits `1`; invalid choices exit `2`, connection failure exits `1` | parser, path resolver, connection and launch services, process, atomic project configuration |
