...
| `mcp connect` | `[PROJECT]`, `--client`, `--mode`, `--print` | current directory, `claude-code`, `live`, off | connects the selected client using the absolute installed CLI path; `--print` only generates its configuration | clients are `claude-code`, `codex`, `cursor`, `vscode`, `json`; modes are `live`, `standard`; Claude Code is project-local, Codex configuration is global, Cursor writes `.cursor/mcp.json`, VS Code writes `.vscode/mcp.json`, and `json` is manual-only; `--print` performs no client process or project write | default stdout describes the result and includes a manual fallback when required; `--print` stdout is only the fragment; invalid choices exit `2`, connection failure exits `1` | parser, path resolver, connection service, process, atomic project configuration |
