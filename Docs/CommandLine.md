...
JSON tree exports use a compact structure:
...
JSON export uses a compact tree format: arrays contain files, objects contain subfolders, `/` contains files in the current folder, and `[]` represents an empty folder. `rootPath` is written once and normalized with `/`; in `tree-content` exports, file content headers use relative `/` paths.
