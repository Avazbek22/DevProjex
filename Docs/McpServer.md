...
When the complete effective manifest proves outgoing file dependencies, the same
`Lines` header appends `Dependencies: [<project-relative paths>]`. The list contains
at most eight resolved paths and at most 512 characters; `omitted=N` reports paths
that did not fit either bound. It does not include ambiguous candidates, unresolved
references, or external symbols, and no dependency suffix is written for an empty
resolved set. Paths are data from the project and remain inside the untrusted block.
