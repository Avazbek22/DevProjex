...
| Network | Local MCP use is offline by default. A built product accepts a repository source only over `https` or `ssh`, including the `scp`-style `user@host:path` form; `http`, `git`, and `file` URLs are refused before Git or the repository cache is reached. That transport set is fixed in the build: no environment variable, profile, configuration file, or command-line value widens it, and an existing local directory stays an ordinary project path rather than a clone transport. Remote Git acquisition requires the startup `--allow-remote` opt-in and then uses the `ExplicitNetwork` Git profile with a validated URL and allowed transport. An optional `--remote-hosts` allowlist narrows that opt-in to exact normalized hosts. Inspection operates on the pinned checkout after acquisition. The trusted trailer reports only a validated lowercase hexadecimal commit id; the repository-controlled branch remains inside structured untrusted data. |
